...
fn unpack_db_entry(db_entry: &str) -> DBEntry {
    let entry_split = db_entry.split(":")
        .collect::<Vec<&str>>();
    let requirements = entry_split[0];
    let password = entry_split[1].trim();
    let requirements = requirements.split(" ").map(|x| x.trim()).collect::<Vec<&str>>();
    let character = requirements[1].trim();
    let counts = requirements[0].split("-")
        .map(|count| count.trim().parse::<i32>().unwrap())
        .collect::<Vec<i32>>();
    DBEntry{min:counts[0], max:counts[1], character:character, password:password}
}

fn db_entries_from_input(input_data: &Vec<String>) -> Vec<DBEntry> {
    input_data.into_iter()
        .map(|db_entry|
            unpack_db_entry(db_entry.as_str())
        ).collect()
}
...
fn is_good_entry_part2(entry: &DBEntry) -> bool {
    (entry.password.chars().nth((entry.min-1) as usize).unwrap() == entry.character.chars().nth(0).unwrap())
        ^ (entry.password.chars().nth((entry.max-1) as usize).unwrap() == entry.character.chars().nth(0).unwrap())
}
